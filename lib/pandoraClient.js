...
pd.toolbarSetup = () => {
	...
	if (php.requestIsYes('debug')) {
		pd.toolbarLeftDOM.append($('<div>').text('Toolbar Left'));
		pd.toolbarCenterDOM.append($('<div>').text('Toolbar Center'));
		pd.toolbarRightDOM.append($('<div>').text('Toolbar Right'));
	}
...
pd.zebraFix = (parentDOM, zebra1, zebra2) => {
	let i = 0;
